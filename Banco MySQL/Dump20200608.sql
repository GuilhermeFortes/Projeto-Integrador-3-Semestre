...
--
-- Table structure for table `superlist`
--

DROP TABLE IF EXISTS `superlist`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `superlist` (
  ...
  `FK_id_aluno1` int DEFAULT NULL,
  `FK_id_aluno2` int DEFAULT NULL,
  `id_aluno3` int DEFAULT NULL,
  `id_aluno4` int DEFAULT NULL,
  `id_aluno5` int DEFAULT NULL,
  PRIMARY KEY (`id_lista`),
  KEY `FK_id_curso` (`FK_id_curso`),
  KEY `FK_id_aluno1` (`FK_id_aluno1`),
  KEY `FK_id_aluno2` (`FK_id_aluno2`),
  KEY `id_aluno3` (`id_aluno3`),
  KEY `id_aluno4` (`id_aluno4`),
  KEY `id_aluno5` (`id_aluno5`),
  CONSTRAINT `superlist_ibfk_1` FOREIGN KEY (`FK_id_curso`) REFERENCES `cursos` (`idcurso`),
  CONSTRAINT `superlist_ibfk_2` FOREIGN KEY (`FK_id_aluno1`) REFERENCES `gafanhotos` (`id`),
  CONSTRAINT `superlist_ibfk_3` FOREIGN KEY (`FK_id_aluno2`) REFERENCES `gafanhotos` (`id`),
  CONSTRAINT `superlist_ibfk_4` FOREIGN KEY (`id_aluno3`) REFERENCES `gafanhotos` (`id`),
  CONSTRAINT `superlist_ibfk_5` FOREIGN KEY (`id_aluno4`) REFERENCES `gafanhotos` (`id`),
  CONSTRAINT `superlist_ibfk_6` FOREIGN KEY (`id_aluno5`) REFERENCES `gafanhotos` (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `superlist`
--

LOCK TABLES `superlist` WRITE;
/*!40000 ALTER TABLE `superlist` DISABLE KEYS */;
INSERT INTO `superlist` VALUES (1,'Lista de Teste','Guilherme Fortes',1,1,2,3,4,5),(2,'Alunos com Dificuldades','Guilherme Fortes',3,1,2,3,4,5),(3,'Alunos de Recuperação','Elisangela Correia',3,22,62,63,14,22);
/*!40000 ALTER TABLE `superlist` ENABLE KEYS */;
UNLOCK TABLES;
...
--
-- Final view structure for view `programadores`
--
...
/*!50001 VIEW `programadores` AS select `gafanhotos`.`id` AS `id`,`gafanhotos`.`nome` AS `nome`,`gafanhotos`.`profissao` AS `profissao`,`gafanhotos`.`nascimento` AS `nascimento`,`gafanhotos`.`sexo` AS `sexo`,`gafanhotos`.`peso` AS `peso`,`gafanhotos`.`altura` AS `altura`,`gafanhotos`.`nacionalidade` AS `nacionalidade` from `gafanhotos` where ((`gafanhotos`.`profissao` = 'Programador') and (`gafanhotos`.`nome` like 'Guilherme')) */;
...
-- Dump completed on 2020-06-08 23:13:48
